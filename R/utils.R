...
#' ... = ggplot objects
#' @export
paste_gg = function(..., fout, save_heights = 5, save_widths = 5, row_heights = NULL, row_widths = NULL, total_height = 5, total_width = 10, nrow, ncol){

  plots = list(...)
  if (length(save_heights) == 1) save_heights = rep(save_heights, length(plots))
  if (length(save_widths) == 1) save_widths = rep(save_widths, length(plots))
...
test_cols = function(cols){
  ...
  if (!is.null(names(cols))) {
    text(rep(1.1, length(cols)), seq_along(cols),
         names(cols),
         col = cols,
         adj = 0)
  }
...
#' @export
add_titles = function(plots, col_titles, row_titles, fontsize = 10, margins = c(.2,.2)){
  ...
  col_title_plots = map(col_titles,
                        ~ gridExtra::arrangeGrob(blank_plot, bottom = grid::textGrob(.x, gp = grid::gpar(
                          fontsize = fontsize
                        ))) %>% ggplotify::as.ggplot())

  row_title_plots = map(row_titles,
                        ~ gridExtra::arrangeGrob(blank_plot,
                                      right = grid::textGrob(
                                        .x,
...
#' @export
add_titles_png = function(pngs, col_titles, row_titles, fontsize = 10, margins = c(.2,.2)){
  plots <- lapply(pngs,
                  function(x){
                    img <- as.raster(png::readPNG(x))
...
to_repr = function(v, newlines = F, v_class = 'infer'){
  ...
  if (v_class == 'infer') v_class = class(v)

  if (!is.null(names(v))){
    w = purrr::map_chr(seq_along(v),
                       function(i){
                         if (names(v)[[i]] != ''){
...
efficient_map = function(l,
  ...
  if (!is.infinite(updates)) catn('Total to do = ', len)

  if (num_cores > 1){
    if (file.exists(outfile)) stop("outfile: ", outfile, ' already exists - stopping to prevent modification')
    ...
    for (i in seq_along(unq_l)){
      if (i %% updates == 0) myUtils::catn(i, ' / ', len)
      o[[i]] = f(unq_l[[i]], ...)
    }
  }
...
compress <- function(to_zip, zip_path, recurse = F, root = '.'){
  myUtils::with_dir(
    dir = root,
    code = {
      cmd = paste0('zip ', ifelse(recurse, ' -rj ', ' -j '), zip_path, ' ', to_zip)
      system(cmd)
      }
    )
}
...
set_ggplot_default_params = function(){
  update_geom_defaults("text", list(family = "Arial", size = default_ggplot_params$textsize_pt_small))
  update_geom_defaults("line", list(linewidth = default_ggplot_params$linewidth))
  update_geom_defaults("hline", list(linewidth = default_ggplot_params$linewidth))
  update_geom_defaults("vline", list(linewidth = default_ggplot_params$linewidth))
  update_geom_defaults("point", list(linewidth = default_ggplot_params$pointsize))
}
...
theme_default = function(){
  theme_classic(
    ...
    theme(
      ...
      legend.text = element_text(size = default_ggplot_params$textsize_pt_small),
